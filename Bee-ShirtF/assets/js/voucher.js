angular.module("voucherApp", []).controller("voucherController", [
  ...
  function ($scope, $http, $timeout) {
    ...
    $scope.getVouchers = function () {
      ...
      $http({
        ...
        .then(function (response) {
          ...
          // Kiểm tra nếu responseData có dạng object với thuộc tính content
    ...
    $scope.searchByDateRange = function () {
      ...
      $http({
          method: "GET",
          url: `http://localhost:8080/voucher/searchByDateRange?batdau=${batdau}&ketthuc=${ketthuc}`,
          headers: {
              Authorization: "Bearer " + token,
          },
      })
      .then(function (response) {
          $scope.voucherList = response.data; // Cập nhật danh sách voucher hiển thị
      })
      .catch(function (error) {
          console.error("Lỗi khi tìm kiếm voucher:", error);
          $scope.errorMessage = "Không thể tìm kiếm voucher.";
    ...
    $scope.updateExpiredVouchers = function () {
      const token = sessionStorage.getItem("jwtToken");

      $http({
        method: "PUT",
        url: "http://localhost:8080/voucher/update-expired-status", // API để cập nhật trạng thái voucher hết hạn
        headers: {
          Authorization: "Bearer " + token,
        },
      })
        .then(function (response) {
          console.log("Cập nhật trạng thái voucher hết hạn thành công");
          // Cập nhật lại danh sách voucher sau khi trạng thái được thay đổi
          $scope.getVouchers();
        })
        .catch(function (error) {
    ...
    if (newVoucher.type_voucher === 'percentage' && (newVoucher.discount_value < 1 || newVoucher.discount_value > 100)) {
        $scope.errorMessage = "Phần trăm giảm giá phải từ 1 đến 100!";
        return;
    }

    if (newVoucher.type_voucher === 'fixed' && newVoucher.discount_value <= 0) {
      ...
      if (!newVoucher.status_voucher) {
        $scope.errorMessage = "Trạng thái voucher là bắt buộc!";
        return;
      }
      ...
      $http({
        ...
        data: {
          ...
          status_voucher: newVoucher.status_voucher || 1
